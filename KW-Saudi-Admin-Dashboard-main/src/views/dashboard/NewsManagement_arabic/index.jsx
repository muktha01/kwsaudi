...
import { newsArabicService } from 'api/news-arabic';
...
const NewsManagement = () => {
  ...
  const fetchNews = async () => {
    try {
      setLoading(true);
  const response = await newsArabicService.getAllNews();
      setNews(response);
    } catch (error) {
  ...
  await newsArabicService.updateNews(selectedNews._id, formDataToSend);
        showMessage('success', 'News updated successfully!');
      } else {
  await newsArabicService.createNews(formDataToSend);
  ...
  const handleDelete = async (newsId) => {
    ...
    try {
  await newsArabicService.deleteNews(newsId);
