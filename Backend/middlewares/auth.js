...
export const authenticateAdmin = async (req, res, next) => {
  try {
    ...
    if (req.cookies.token) {
      ...
      // Support custom header for reverse proxy scenarios
      token = req.headers['x-auth-token'];
      console.log('Token found in x-auth-token header');
    }

    if (!token) {
      console.log('No token provided - cookies:', !!req.cookies.token, 'auth header:', !!req.headers.authorization, 'origin:', req.get('origin'));
