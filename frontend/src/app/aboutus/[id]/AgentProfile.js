...
const AgentProfile = (props) => {
  ...
  // Add loading state for download buttons if needed
...
const [buyEmail, setBuyEmail] = useState("");
  ...
  //         setError(t('Unable to connect to the server. Please check your internet connection and try again.'));
  //       } else if (e.message.includes('API failed')) {
  //         setError(t('Server is currently unavailable. Please try again later.'));
  //       } else {
  //         setError(t('Failed to load properties: ') + e.message);
  //       }
...
<div className="relative bg-gray-100 px-4 lg:px-20 py-4 lg:py-10">
  ...
  {/* Agent Card Section */}
<div className="hidden lg:flex flex-col md:flex-row md:mt-10 lg:mt-40 mt-20 shadow-xl rounded-3xl overflow-hidden w-full">
  {/* Left Section */}
  ...
  <div className="w-full md:w-1/2 relative flex items-center justify-center bg-[rgb(206,32,39,255)] md:bg-transparent lg:bg-transparent mt-8 md:mt-0 lg:mt-0">
    ...
    <div className="relative z-10 rounded-full overflow-hidden border-4 border-white shadow-xl bg-gray-100 w-48 h-48 sm:w-56 sm:h-56 md:w-72 md:h-72 lg:w-96 lg:h-96 aspect-square">
      <Image
        ...
        alt={t(`Portrait of ${agent.name || "Agent"}`)}
        fill
...
<div className="flex flex-col bg-[rgb(206,32,39,255)] text-white shadow-lg rounded-2xl gap-4 mt-30 lg:mt-4 lg:hidden">
  {/* Agent Image */}
