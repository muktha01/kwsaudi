...
export default function Aboutus() {
  ...
  useEffect(() => {
    const CACHE_KEY = 'aboutus_page_data';
    const CACHE_EXPIRY_KEY = 'aboutus_page_data_expiry';
    const SESSION_CACHE_KEY = 'aboutus_page_session';
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      try {
        // Check sessionStorage first for ultra-fast access
        const sessionData = sessionStorage.getItem('aboutus_page_session');
        if (sessionData) {
          const parsedData = JSON.parse(sessionData);
        ...
        const cachedData = localStorage.getItem('aboutus_page_data');
        const cachedExpiry = localStorage.getItem('aboutus_page_data_expiry');
        const now = Date.now();
        if (cachedData && cachedExpiry && now < parseInt(cachedExpiry)) {
          const parsedData = JSON.parse(cachedData);
          if (parsedData.page && !page) setPage(parsedData.page);
