...
const Franchise = () => {
  ...
  const selectedNationality = nationalityList.find(n => n.value === form.nationality) || nationalityList[0];
  ...
  useEffect(() => {
    const CACHE_KEY = 'franchise_page_data';
    const CACHE_EXPIRY_KEY = 'franchise_page_data_expiry';
    const SESSION_CACHE_KEY = 'franchise_page_session';
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      try {
        // Check sessionStorage first for ultra-fast access
        const sessionData = sessionStorage.getItem('franchise_page_session');
        if (sessionData) {
          const parsedData = JSON.parse(sessionData);
        ...
        const cachedData = localStorage.getItem('franchise_page_data');
        const cachedExpiry = localStorage.getItem('franchise_page_data_expiry');
        const now = Date.now();
        if (cachedData && cachedExpiry && now < parseInt(cachedExpiry)) {
          const parsedData = JSON.parse(cachedData);
          if (parsedData.page && !page) setPage(parsedData.page);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const selectedNationalityObj = nationalityList.find(n => n.value === form.nationality) || nationalityList[0];
  ...
  <ul className="mt-6 space-y-6 text-sm">
    ...
    />
            ...
            <div className="mb-2">
              <label className="block text-sm mb-2 font-medium text-gray-700">{t("Nationality")}</label>
              {mounted && (
                <Select
              options={nationalityList}
              value={selectedNationality}
              onChange={(option) =>
                setForm((prev) => ({ ...prev, nationality: option.value }))
              }
