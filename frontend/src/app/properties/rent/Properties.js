...
const PropertiesContent = () => {
  ...
  const fetchPropertiesWithCache = useCallback(async (page, requestBody) => {
    const cacheKey = `rent-properties-${JSON.stringify(requestBody)}-page-${page}`;
    
    // Check cache first
    if (cacheRef.current.has(cacheKey)) {
      // console.log('Using cached rent properties data for:', cacheKey);
  ...
  const fetchProperties = useCallback(async (page = 1) => {
    ...
    // Create cache keys
    const PROPERTIES_CACHE_KEY = `rent_properties_${JSON.stringify(requestBody)}_page_${page}`;
      ...
      try {
        ...
        if (countRes.ok) {
          ...
          if (countData.total !== parseInt(cachedCount || '0')) {
            console.log('🔄 New rent properties detected, refreshing cache...');
      ...
      try {
        ...
        if (data.success) {
          ...
          if (isBackgroundUpdate) {
            console.log('✅ Rent properties updated with latest data');
        ...
        console.error('Error fetching fresh rent properties:', err);
        if (!isBackgroundUpdate) {
  ...
  useEffect(() => {
    const CACHE_KEY = 'rent_page_data';
    const CACHE_EXPIRY_KEY = 'rent_page_data_expiry';
    const SESSION_CACHE_KEY = 'rent_page_session';
    ...
    const fetchPageHero = async () => {
      try {
        ...
        if (error.name === 'AbortError') {
          console.warn('Rent page fetch timeout');
        }
        ...
        if (typeof window !== 'undefined') {
          ...
          if (cachedData) {
            try {
              ...
              console.warn('Error parsing cached rent page data:', parseError);
            }
          }
        }
      }
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      try {
        // Check sessionStorage first for ultra-fast access
        const sessionData = sessionStorage.getItem('rent_page_session');
        if (sessionData) {
          const parsedData = JSON.parse(sessionData);
        ...
        const cachedData = localStorage.getItem('rent_page_data');
        const cachedExpiry = localStorage.getItem('rent_page_data_expiry');
        const now = Date.now();
        if (cachedData && cachedExpiry && now < parseInt(cachedExpiry)) {
          const parsedData = JSON.parse(cachedData);
          if (parsedData.page && !page) setPage(parsedData.page);
          ...
          sessionStorage.setItem('rent_page_session', cachedData);
        }
      } catch (e) {
        console.warn('Error reading cached rent page data in client effect:', e);
      }
    }
  ...
  <div className="relative bg-gray-100 md:pb-10">
    ...
    <section className={`relative w-full ${showFilters ? 'h-[120vh] md:h-[125vh]' : 'h-screen md:h-screen'} text-white overflow-hidden transition-all duration-500 ease-in-out`}>
      ...
      <Image
                src={heroSrc}
              alt="Rent Properties Hero Background"
              layout="fill"
