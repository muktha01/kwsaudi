// app/properties/newdevelopment/page.js
...
export async function generateMetadata({ searchParams }) {
  ...
  try {
    const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/seo/slug/new-development`, { next: { revalidate: 60 } });
    ...
    return {
      ...
      ...generateHreflangMetadata('/properties/newdevelopment', lang), // Add hreflang tags
    };
  } catch (err) {
    // Fallback with hreflang even if SEO API fails
    console.error('Failed to fetch SEO:', err);
    return {
      ...generateHreflangMetadata('/properties/newdevelopment', lang),
    };
  }
}
