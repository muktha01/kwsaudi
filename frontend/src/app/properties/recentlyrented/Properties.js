...
const applySorting = useCallback((propertiesArray) => {
  ...
  useEffect(() => {
    async function fetchProperties(page = 1) {
      ...
      try {
        ...
        // console.log('fetchProperties requestBody with sorting ->', JSON.stringify(requestBody));
        ...
        if (data.success) {
          ...
          // Debug: Check what data we're getting from API
          // console.log('Raw data from API:', fetched.length, 'properties');
          if (fetched.length > 0) {
            // console.log('Sample raw property data:', fetched.slice(0, 3).map(p => ({
  ...
  }, [currentPage, perPage, appliedFilters, sortOption, fetchPropertiesWithCache, applySorting]); // sortOption added back for backend sorting
